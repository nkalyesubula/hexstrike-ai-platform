...
const METASPLOITABLE_PRESET_TOOLS = ["nmap", "nikto", "gobuster", "enum4linux-ng", "hydra"];

const normalizeHostTarget = (value) => {
...
const buildMetasploitablePresetParameters = (target) => {
	const host = normalizeHostTarget(target);
	if (!host) {
		addOutput("❌ Invalid target", "error");
		return;
	}
	...
	return {
		...
		nikto: {
			target: webUrl,
			additional_args: "-C all",
		},
		gobuster: {
			url: webUrl,
			mode: "dir",
			wordlist: "/usr/share/wordlists/dirb/common.txt",
			additional_args: "-x php,html,txt -t 20",
		},
		"enum4linux-ng": {
			shares: true,
			users: true,
...
function LabPage() {
	...
	const startPentest = async () => {
		...
		try {
			const response = await fetchWithAuth("/api/tools/batch", {
				method: "POST",
				body: JSON.stringify({ target, tools: selectedTools, auto_mode: true, parameters: toolParameters }),
			});
			const data = await response.json();
	...
	const applyMetasploitablePreset = () => {
		const presetTarget = normalizeHostTarget(target);
		setTarget(presetTarget);
		setSelectedTools(METASPLOITABLE_PRESET_TOOLS);
		setToolParameters(buildMetasploitablePresetParameters(presetTarget));
		setSelectedCategory("all");
		setSearchTerm("");
		addOutput(`Loaded Metasploitable 2 preset for ${presetTarget}`, "success");
		addOutput(`Tools: ${METASPLOITABLE_PRESET_TOOLS.join(", ")}`, "info");
	};
		...
		<div style={{ padding: "40px", background: "#1a1a2e", minHeight: "100vh", color: "white" }}>
			...
			{isRunning && currentProgress && (
				<div style={{ background: "#16213e", padding: "15px", borderRadius: "12px", marginBottom: "20px" }}>
					<div style={{ display: "flex", justifyContent: "space-between", marginBottom: "10px" }}>
						<span>Progress</span>
						<span>
							{currentProgress.current_tool + 1}/{currentProgress.total_tools} tools
						</span>
					</div>
			...
			<div
				style={{
					background: "#16213e",
					border:
						selectedTools.join(",") === METASPLOITABLE_PRESET_TOOLS.join(",")
							? "1px solid #10b98170"
							: "1px solid #6c63ff35",
					padding: "18px 20px",
					borderRadius: "12px",
			...
			>
				...
				>
					<div>
						<h3 style={{ margin: "0 0 6px", fontSize: "17px", fontWeight: "bold" }}>Metasploitable 2 Quick Pentest</h3>
						<div style={{ color: "#a5b4fc", fontSize: "13px" }}>nmap, nikto, gobuster, enum4linux-ng, hydra</div>
					</div>
					<button
						onClick={applyMetasploitablePreset}
						disabled={isRunning}
						style={{
							background: "#10b981",
							color: "#06131c",
				...
				<div style={{ marginTop: "12px", color: "#94a3b8", fontSize: "12px", lineHeight: 1.5 }}>
					Uses HTTP for web tools, SMB null-session enumeration, and a controlled SSH Hydra check with
					msfadmin/msfadmin.
				</div>
			</div>
							...
							filteredTools.map(([toolId, tool]) => (
								<label
									...
									style={{
										...
										padding: "12px",
										marginBottom: "8px",
										background: selectedTools.includes(toolId) ? "#6c63ff20" : "#0f3460",
										borderRadius: "8px",
										cursor: "pointer",
									}}
								>
					...
					<div style={{ maxHeight: "400px", overflow: "auto" }}>
						{sessions.length === 0 ? (
							...
							sessions.map((session) => (
								<div
									key={session.id}
									style={{ background: "#0f3460", padding: "12px", borderRadius: "8px", marginBottom: "10px" }}
								>
									...
									<div style={{ fontSize: "12px", color: "#888", marginTop: "5px" }}>
										{new Date(session.started_at).toLocaleString()} | {session.findings_count} findings
									</div>
								</div>
							))
						)}
				...
				>
					{output.length === 0 ? (
						...
						output.map((line, idx) => {
							...
							return (
								<div key={idx} style={{ marginBottom: "6px", color: color, whiteSpace: "pre-wrap" }}>
